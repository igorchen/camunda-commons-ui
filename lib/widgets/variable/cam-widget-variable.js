define([
  ...
  function (
    ...
    $dialogScope.deserialize = function () {
      $http({
        method: 'GET',
        url: Uri.appUri('engine://engine/:engine/variable-instance/'+ variable.id)
      })
      .success(function(data) {
        if(!data.errorMessage) {
          $dialogScope.inspect.initial = JSON.stringify(data.value);
          $dialogScope.inspect.current = angular.copy($dialogScope.inspect.initial);
        }
        else {
          $scope.inspect.error = data.errorMessage;
        }
      })
      .error(function(data) {
        $dialogScope.inpect.deserialized = data;
      });
    };
    ...
    $dialogScope.typeIn = function(formScope, type) {
      if(type === 'serialized') {
        ...
        $dialogScope.inspect.current = formScope.currentValueDeserialized;
      }
